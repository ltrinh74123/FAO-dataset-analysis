...
```{r}
emissions = read_csv("data/Emissions_Totals_E_All_Data_(Normalized).csv")
emissions$Item %>% unique()
emissions$Element %>% unique()
distinct(emissions, Item, `Item Code`)
```

```{r warning=FALSE}

setwd('/Users/linh/Documents/Github/FAO-dataset-analysis/data/emissions')
emissions = list.files(pattern="*.csv")
myfiles = lapply(emissions, read.delim)
```
